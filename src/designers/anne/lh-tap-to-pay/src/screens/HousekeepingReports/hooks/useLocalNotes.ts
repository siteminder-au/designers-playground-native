...
const STORAGE_KEY = 'anne.lh-tap-to-pay.roomNotes.v1';
...
export function useLocalNotes() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY)
      ...
      .catch(err => console.warn('[anne] load room notes failed', err))
  ...
  useEffect(() => {
    ...
    AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(notes))
      .catch(err => console.warn('[anne] save room notes failed', err));
  }, [notes]);
